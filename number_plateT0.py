...
plates_dir = r"C:\Users\Rahil Kumar\OneDrive\Desktop\Number Plate Detection\Number Plate\Number Plate\plates"
excel_path = r"C:\Users\Rahil Kumar\OneDrive\Desktop\Number Plate Detection\Number Plate\Number Plate\Detected Plates.xlsx"
video_path = r"C:\Users\Rahil Kumar\OneDrive\Desktop\Number Plate Detection\Number Plate\Number Plate\mycarplate.mp4"  # Path to the input video file
...
# Load the YOLO model
model = YOLO(r'Number Plate\Number Plate\model\best.pt')
...
while cap.isOpened():
    ...
    for result in results:
        for box in result.boxes:
            ...
            # Get OCR text
            ocr_texts = get_ocr_text(frame, (xmin, ymin, xmax, ymax))
            print(f"OCR Texts: {ocr_texts}")

            # Validate and find the most common valid plate text
            valid_texts = [text.replace(" ", "").upper() for text in ocr_texts if is_valid_plate(text)]
            if valid_texts:
                most_common_text = Counter(valid_texts).most_common(1)[0][0]

                # Only save if it's a new plate or more confident detection
                if most_common_text not in detected_plates or detected_plates[most_common_text] < confidence:
                    detected_plates[most_common_text] = confidence

                    # Draw the bounding box and extracted text on the frame
                    cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)
                    cv2.putText(frame, most_common_text, (xmin, ymax + 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 0, 0), 2)
                    ...
                    save_to_excel(img_path, most_common_text)
                    count += 1
